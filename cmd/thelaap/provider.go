...
type ProviderState struct {
	...
	Offerti  []OfferedModel `json:"offerti"`
}
...
// readProviders raccoglie i provider dai file dei client.
//
...
func readProviders() []credProvider {
	...
	for chiave, pv := range provs {
		...
		out = append(out, credProvider{
			...
			apiKey: key, remoto: !addressOfThisMachine(base),
		})
	}
...
// provRemote: il provider gira su un'altra macchina?
//
...
func provRemote(chiave string) bool {
	cacheRemotoMu.RLock()
	c := cacheRemoto
...
// forgetRemote invalida la cache dopo una scrittura delle configurazioni.
func forgetRemote() {
	cacheRemotoMu.Lock()
	cacheRemoto = nil
	cacheRemotoMu.Unlock()
...
func addressOfThisMachine(u string) bool {
	l := strings.ToLower(u)
...
func interroga(c credProvider, configurati map[string]bool) ProviderState {
	s := ProviderState{Chiave: c.chiave, Nome: c.nome, Remoto: c.remoto, Offerti: []OfferedModel{}}
	...
	for _, m := range corpo.Data {
		...
		s.Offerti = append(s.Offerti, OfferedModel{
...
func apiProvider(w http.ResponseWriter, r *http.Request) {
	...
	provs := readProviders()
	out := make([]ProviderState, len(provs))
		...
		go func(i int, c credProvider) {
			...
			defer func() {
				if rec := recover(); rec != nil {
					out[i] = ProviderState{Chiave: c.chiave, Nome: c.nome,
						Remoto: c.remoto, Errore: "errore interno", Offerti: []OfferedModel{}}
				}
			}()
...
// destinationFor: dove mandare una richiesta per un dato provider.
//
...
func destinationFor(chiave string) (destinazione, bool) {
	for _, p := range readProviders() {
		if p.chiave == chiave {
