...
func processFootprint(pid int) (Footprint, error) {
	...
	if err != nil {
		return Footprint{}, fmt.Errorf("Get-Process per il pid %d: %w", pid, err)
	}
	...
	if corrente == 0 {
		return Footprint{}, fmt.Errorf("misura nulla per il pid %d", pid)
	}
	return Footprint{CorrenteByte: corrente, PiccoByte: picco, Stimato: true}, nil
}
